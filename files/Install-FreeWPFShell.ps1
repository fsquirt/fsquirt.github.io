...
if (-not (Test-Path $destFolder)) {
    Write-Host "正在创建目录: $destFolder..." -ForegroundColor Cyan
    New-Item -Path $destFolder -ItemType Directory | Out-Null
}

# 3. 下载文件
Write-Host "正在从 $url 下载文件..." -ForegroundColor Cyan
Invoke-WebRequest -Uri $url -OutFile $zipFile

# 4. 解压文件
Write-Host "正在解压到 $destFolder..." -ForegroundColor Cyan
# 使用 -Force 确保覆盖旧文件
...
# 7. 启动程序
Write-Host "安装完成！正在启动 FreeWPFShell..." -ForegroundColor Green
Start-Process -FilePath $exePath -WorkingDirectory $destFolder

Write-Host "一切就绪！" -ForegroundColor Yellow